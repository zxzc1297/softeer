...
public class boj_5002 {
    public static void main(String[] args) {
        ...
        int X = sc.nextInt(); // 최대 허용 남녀 차이
        String line = sc.next(); // 대기열 (W: 여성, M: 남성)
        sc.close();

        int men = 0, women = 0; // 현재 클럽에 입장한 남녀 수
        StringBuilder queue = new StringBuilder(line); // 대기열

        while (queue.length() > 0) {
            char first = queue.charAt(0); // 첫 번째 사람
            char second = queue.length() > 1 ? queue.charAt(1) : ' '; // 두 번째 사람 (없으면 공백)

            // 첫 번째 사람을 바로 입장 가능하면 입장
            if (canEnter(men, women, first, X)) {
                if (first == 'M') men++;
                else women++;
                queue.deleteCharAt(0);
            }
            // 첫 번째 사람을 입장시키면 남녀 차이를 초과할 경우, 두 번째 사람 체크
            else if (second != ' ' && canEnter(men, women, second, X)) {
                if (second == 'M') men++;
                else women++;
                queue.deleteCharAt(1);
            }
            // 아무도 입장 불가능하면 종료
            else {
                break;
    ...
    // 현재 남녀 수와 입장할 성별이 허용된 차이를 넘지 않는지 확인
    public static boolean canEnter(int men, int women, char person, int X) {
        if (person == 'M') return Math.abs((men + 1) - women) <= X;
        else return Math.abs(men - (women + 1)) <= X;
    }
}
