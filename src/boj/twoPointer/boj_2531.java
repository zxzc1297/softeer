...
public class boj_2531 {
    public static void main(String[] args) {
        ...
        // 슬라이딩 윈도우를 위한 준비
        int[] sushiCount = new int[d + 1]; // 각 초밥의 먹은 개수
        int uniqueSushi = 0; // 현재 먹고 있는 초밥의 종류 수
        int maxSushi = 0; // 최대로 먹을 수 있는 초밥의 종류 수

        // 초기 윈도우 설정 (첫 k개의 초밥)
        for (int i = 0; i < k; i++) {
            if (sushiCount[belt[i]] == 0) uniqueSushi++; // 새로운 종류 추가
            sushiCount[belt[i]]++;
        }

        // 쿠폰 초밥 추가
        maxSushi = uniqueSushi + (sushiCount[c] == 0 ? 1 : 0);
        ...
        for (int i = 1; i < n; i++) {
            ...
            // 윈도우의 첫 번째 초밥 제거
            sushiCount[belt[i - 1]]--;
            if (sushiCount[belt[i - 1]] == 0) uniqueSushi--; // 종류 감소

            // 윈도우의 끝 초밥 추가
            if (sushiCount[belt[end]] == 0) uniqueSushi++; // 새로운 종류 추가
            sushiCount[belt[end]]++;

            // 쿠폰 초밥 고려하여 최댓값 갱신
            maxSushi = Math.max(maxSushi, uniqueSushi + (sushiCount[c] == 0 ? 1 : 0));
        }
