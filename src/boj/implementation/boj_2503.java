...
public class boj_2503 {
    ...
    static int[] check(int candidate, int guess) {
        String cand = String.valueOf(candidate);
        String ques = String.valueOf(guess);

        int strike = 0, ball = 0;
        for (int i = 0; i < 3; i++) {
            if (cand.charAt(i) == ques.charAt(i)) {
                strike++;
            } else if (cand.contains(String.valueOf(ques.charAt(i)))) {
                ball++;
            }
        }
