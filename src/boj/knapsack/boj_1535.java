...
public class boj_1535 {
    public static void main(String[] args) {
        ...
        int[] healthLoss = new int[N + 1]; // 체력 소모 배열
        int[] happinessGain = new int[N + 1]; // 기쁨 증가 배열

        for (int i = 1; i <= N; i++) {
            healthLoss[i] = scanner.nextInt();
        }
        for (int i = 1; i <= N; i++) {
            happinessGain[i] = scanner.nextInt();
        }

        // DP 배열: dp[체력] = 기쁨의 최대값
        int[] dp = new int[101]; // 체력은 100까지 가능

        // DP 계산
        for (int i = 1; i <= N; i++) {
            for (int j = 100; j > 0; j--) {
                if (j > healthLoss[i]) { // 현재 체력으로 해당 사람에게 인사 가능한 경우
                    dp[j] = Math.max(dp[j], dp[j - healthLoss[i]] + happinessGain[i]);
                }
            }
        }
